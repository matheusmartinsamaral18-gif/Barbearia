...
export default function App() {
  ...
  const [adminTab, setAdminTab] = useState<'pending' | 'today' | 'upcoming' | 'all' | 'config'>('pending');
  ...
  const updateStatus = async (id: string, status: AppointmentStatus, extraData = {}) => {
    try {
      ...
      if (app && app.deviceToken) {
        ...
        let body = `Seu status mudou para: ${getStatusLabel(status)}`;

        if (status === 'aceito') {
           title = "Agendamento Aceito! ✅";
           body = `Seu horário de ${app.time} foi confirmado.`;
        } else if (status === 'sugestao_enviada_admin') {
           title = "Nova Sugestão 🕒";
